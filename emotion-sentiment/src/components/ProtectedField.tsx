...
export default function ProtectedField({ value, errShow, ...inputProps }: any) {
    ...
    return (
      <div className="w-full">
        <div className={`mb-1 relative rounded-md w-full flex flex-row border ${errShow ? 'border-red-500' : 'border-gray-300'}  transition duration-100 ease-linear`}>
          ...
          <input {...inputProps} type={visible ? 'text': 'password'} value={value} className={`focus:outline-none block w-full py-2 pl-12 sm:text-sm rounded-md ${errShow ? 'text-red-600': 'text-black'}`} />
